...
from .models import DIFFICULTY_VOICE_MAP
...
class TrainingChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        # FIX: pick the TTS voice based on difficulty so each persona sounds distinct
        self.voice = DIFFICULTY_VOICE_MAP.get(self.session['difficulty'], 'alloy')
    ...
    async def _handle_agent_message(self, agent_text: str):
        """Add agent message to history, call OpenAI, stream response back as text + audio."""
        self.turn_count += 1
        ...
        # FIX: This was previously missing entirely — the AI reply was only ever
        # sent back as raw text, so Flutter had nothing to play as audio.
        # Now we generate speech via OpenAI TTS and send it back as base64,
        # exactly the same shape as how the user's audio came in.
        ai_audio_b64 = None
        try:
            ai_audio_b64 = await self._text_to_speech(ai_text)
        except Exception as e:
            logger.exception(f"TTS error: {e}")
            # Don't hard-fail the whole turn if TTS breaks — still send text
            # back so the conversation can continue, just flag it.

        await self.send(text_data=json.dumps({
            'type': 'ai_response',
            'text': ai_text,
            'audio_base64': ai_audio_b64,  # <-- Flutter plays this
    ...
    async def _text_to_speech(self, text: str) -> str:
        """Convert AI text response to speech audio, return as base64 string."""
        client = openai.AsyncOpenAI(api_key=settings.OPENAI_API_KEY)

        response = await client.audio.speech.create(
            model='tts-1',       # use 'tts-1-hd' for higher quality if latency allows
            voice=self.voice,    # set per-difficulty in connect()
            input=text,
            response_format='mp3',
        )
